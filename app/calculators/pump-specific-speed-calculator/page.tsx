...
export default function PumpSpecificSpeedCalculator() {
  ...
  const validateHead = (value: string) => {
    ...
    if (isNaN(num)) {
      ...
      toast({ title: "Invalid Input", description: "Head must be a positive number.", variant: "destructive" })
    } else {
      setHeadError("")
    }
  }
  ...
  const handleCalculate = () => {
    ...
    const Q_SI = convertToSI(Q_input, flowUnit, 'flow') // Standard metric context expects m³/h
    const H_SI = convertToSI(H_input, headUnit, 'head') // Standard metric context expects m

    // Step 2: Calculate Ns -> Ns = (N × √Q) / (H^0.75)
    ...
    const steps =[
      ...
      `  Ns = (N × √Q) / H^(3/4)`,
      `  Ns = (${N_input} × √${Q_SI.toFixed(2)}) / ${H_SI.toFixed(2)}^(3/4)`,
      `  Ns = (${N_input} × ${sqrt_Q.toFixed(4)}) / ${denominator.toFixed(3)}`,
      `  Ns ≈ ${ns_exact.toFixed(2)}`
    ]

    setResult({
      value: Math.round(ns_exact).toString(), // Rounded integer for main display
      fullValue: ns_exact.toFixed(2),         // 2 decimals for step display
      sqrtQ: sqrt_Q.toFixed(4),
      denominator: denominator.toFixed(3),
      calculated: true,
      steps
    })
          ...
          <div className="p-4 space-y-4 flex flex-col flex-1">
            ...
            <div>
              <div className="flex items-center gap-2.5">
                <label className="font-semibold text-foreground text-xs w-28 flex-shrink-0">Rotational Speed (N):</label>
                <input
                  type="number"
                  value={speed}
            ...
            <div>
              <div className="flex items-center gap-2.5">
                <label className="font-semibold text-foreground text-xs w-28 flex-shrink-0">Flow Rate (Q):</label>
                <input
                  type="number"
                  value={flowRate}
            ...
            <div>
              <div className="flex items-center gap-2.5">
                <label className="font-semibold text-foreground text-xs w-28 flex-shrink-0">Total Head (H):</label>
                <input
                  type="number"
                  value={head}
            ...
            <div className="mt-auto mb-3 bg-muted/40 rounded-lg p-3 border border-border flex flex-col items-center shadow-sm">
              ...
              <div className="font-serif text-lg flex items-center gap-2.5">
                <span className="font-bold italic">N<sub className="text-[11px] not-italic">s</sub></span>
                <span>=</span>
                <div className="flex flex-col items-center">
                  ...
                  <span className="pt-1 text-[15px] font-medium">
                    H<sup className="text-[10px] ml-0.5">3/4</sup>
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-background rounded-md border border-border overflow-hidden shadow-sm">
              ...
              <div className="p-2.5">
                <div className="inline-flex items-center bg-blue-50 dark:bg-blue-950/30 border border-blue-400 dark:border-blue-600 rounded px-2 py-1 font-mono text-xs shadow-sm">
                  <strong className="text-blue-600 dark:text-blue-400 mr-1">N<sub className="text-[9px]">s</sub></strong> 
            ...
            <div className="bg-background rounded-md border border-border overflow-hidden shadow-sm transition-all">
              ...
              <div className="border-b border-border">
                ...
                {showStep1 && (
                  <div className="px-4 pb-3 space-y-1 text-xs font-mono bg-blue-50/50 dark:bg-blue-950/20 pt-1">
                    ...
                    <div>H = {head || "?"} {headUnits.find(u => u.value === headUnit)?.label} = {result.calculated ? (result.steps[3]?.split('=')[2]?.trim() || "?") : "?"}</div>
                  </div>
                )}
              </div>
              ...
              <div className="p-4 overflow-x-auto">
                ...
                <div className="grid grid-cols-[auto_auto_1fr] items-center gap-y-5 gap-x-3 font-serif text-base min-w-max">
                  ...
                  {/* General Formula */}
                  <span className="font-bold italic text-right whitespace-nowrap">N<sub className="text-[11px] not-italic font-bold">s</sub></span>
                  ...
                  <div className="flex flex-col items-center justify-self-start">
                    ...
                    <span className="pt-1 text-sm whitespace-nowrap">
                      H<sup className="text-[9px] ml-0.5">3/4</sup>
                    </span>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center justify-self-start gap-y-2">
                    ...
                    <div className="flex flex-col items-center">
                      ...
                      <span className="pt-1 text-sm whitespace-nowrap">
                        <span className={`bg-yellow-100 dark:bg-yellow-900/60 dark:text-yellow-100 px-1 py-0.5 rounded font-sans ${!result.calculated && 'italic font-medium text-muted-foreground bg-transparent border border-dashed border-muted-foreground'}`} title="H (Total Head in m)">
                          {result.calculated ? (result.steps[3]?.split('=')[2]?.replace('m', '')?.trim() || "H") : "H"}
                        </span>
                  ...
                  {/* Intermediate Math Step -> e.g. (100 * 12.2474) / 31.623 */}
                  <span></span>
                  <span className="text-center">=</span>
                  <div className="flex flex-col items-center justify-self-start font-sans font-medium">
                    <span className="border-b border-foreground px-3 pb-1 text-sm whitespace-nowrap">
                      {result.calculated ? `${speed} × ${result.sqrtQ}` : "?"}
                    </span>
                    <span className="pt-1 text-sm whitespace-nowrap">
                 ...
                 {result.calculated ? (
                   <div className="flex items-center justify-center gap-3 overflow-hidden w-full px-8">
                     ...
                     <div className="flex items-center gap-2.5 min-w-0">
                       ...
                       <div className="text-2xl font-black truncate min-w-0" title={`Ns = ${result.value}`}>
                         N<sub className="text-[14px] font-bold">s</sub> = {formatDisplayNumber(result.value)}
                       </div>
                     </div>
                   </div>
                 ) : (
