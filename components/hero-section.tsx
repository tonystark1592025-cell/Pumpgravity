...
import { 
  ...
  Sigma, ArrowRight, Activity, MoveDown, Sparkles
} from "lucide-react"
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid items-center gap-16 lg:grid-cols-2">
          ...
          <div className="relative flex items-center justify-center">
            ...
            <div className="relative h-[500px] w-full max-w-sm rounded-[2rem] border border-border/50 bg-background/60 p-4 shadow-2xl backdrop-blur-xl">
              ...
              <div className="relative flex h-full w-full flex-col overflow-hidden rounded-[1.5rem] bg-muted/60 ring-1 ring-border/50">
                
                {/* --- TOP CHAMBER (INPUT) --- */}
                <div className="relative flex h-[40%] w-full flex-col items-center justify-center border-b border-border/30 bg-gradient-to-b from-muted/20 to-transparent">
                  {/* Measurement Ticks */}
                  <div className="absolute left-2 top-0 h-full w-4 flex flex-col justify-evenly opacity-30">
                    {[...Array(5)].map((_, i) => <div key={i} className="h-[1px] w-full bg-foreground" />)}
                  </div>
                  ...
                  <AnimatePresence mode="wait">
                    ...
                    >
                      ...
                      <div className="mt-1 text-3xl font-bold text-foreground font-mono tracking-tight">{activeConverter.input}</div>
                    </motion.div>
                  </AnimatePresence>
                ...
                <div className="relative z-20 flex h-[20%] w-full items-center justify-center">
                  {/* Glass Housing */}
                  <div className="absolute inset-x-4 h-16 rounded-xl border border-border/30 bg-muted/20 backdrop-blur-md" />
                  
                  {/* Flow Direction Arrow */}
                  <div className="absolute inset-0 flex items-center justify-center opacity-10">
                     <MoveDown className="h-24 w-24 animate-pulse text-foreground" />
                  </div>
                  ...
                  <motion.div
                    ...
                    className={`relative flex h-20 w-20 items-center justify-center rounded-full bg-gradient-to-br ${activeConverter.liquid} shadow-[0_0_30px_rgba(0,0,0,0.3)] ring-4 ring-background/80`}
                  >
                ...
                <div className="relative flex h-[40%] w-full flex-col items-center justify-center border-t border-border/30">
                   ...
                   <AnimatePresence mode="wait">
                     ...
                     >
                        ...
                        {/* Wave Top */}
                        <div className="absolute -top-1 left-0 right-0 h-2 bg-white/30 blur-sm" />
                     </motion.div>
                   </AnimatePresence>
                   ...
                   <AnimatePresence mode="wait">
                    ...
                    >
                      <div className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">Calculated Output</div>
                      <div className="mt-1 text-4xl font-bold text-foreground font-mono drop-shadow-lg">{activeConverter.output}</div>
                      <Badge variant="secondary" className="mt-2 bg-muted/60 text-foreground backdrop-blur">
